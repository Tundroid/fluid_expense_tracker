...
@app_views.route("/get/<model>/<depot_id>", methods=['GET'], strict_slashes=False)
@jwt_required()
def get_depot_items(model=None, model_id=None):
    ...
    # Perform the join
    results = (
        db.session.query(Depot, Item)
        .join(Item, Depot.item_id == Item.id)
        .all()
    )
    ...
    data = [
        {
            "depot_id": depot.depot,
            "item_id": item.id,
            "item_name": item.name,
            "stock": depot.stock
        }
        for depot, item in results
    ]
